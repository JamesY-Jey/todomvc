...
import { initTcb } from '@/utils';
import config from '@/utils/config';
import API from '@/api';

// 初始化tcb
const tcb = initTcb({ envId: config.envId });
console.log('tcb ===>', tcb);
...
const { todos, newTodo, beforeEditCache, editedTodo, visibility, currentFileID } = toRefs(state);
...
async function getTodos() {
  ...
  console.log('fetchTodo ===>', res);
  todos.value = res.result.data || [];
}
...
async function addTodo() {
  ...
  const res = await API.addTodo(newTodoData);
  state.currentFileID = '';
...
async function removeTodo(todo) {
  ...
  const res = await API.removeTodo(todo.id)
  console.log('removeTodo res ===>', res);
}
...
async function removeCompleted() {
  ...
  const res = await API.removeCompleted();
  console.log('removeCompleted ===>', res);
}

// 改变单项完成状态
async function completedChange(id) {
  const todo = state.todos.find(todo => todo.id === id);
  const res = await API.updateTodo({
    id,
    completed: todo.completed
  });
  console.log('completedChange res ===>', res);
}

// 改变所有完成状态
async function completedChangeAll(value) {
  const res = await API.changeAllState(value);
  console.log('completedChangeAll res ===>', res);
}

function uploadSuccessHandle(params) {
  console.log('uploadSuccessHandle ===> ', params);
  currentFileID.value = params.fileID;
}

function uploadSuccessHandleItem(params) {
  console.log('uploadSuccessHandleItem ===> ', params);
  const item = state.todos.find(todo => todo.id === params.id);
  item.fileID = params.fileID;
...
<template>
  <main class="todo-app">
    <header class="todo-header">
      ...
      <File :fileID="currentFileID" :type="currentFileID ? 1: 0" @uploadSuccess="uploadSuccessHandle" />
    </header>
    <section class="todo-main" v-show="todos.length">
      ...
      <ul class="todo-list">
        <li class="todo-item" v-for="todo in filteredTodos" :key="todo.id" :class="{ completed: todo.completed, editing: todo == editedTodo }">
          <div class="view">
            <input class="toggle" type="checkbox" v-model="todo.completed" @change="completedChange(todo.id)" />
            <label @dblclick="editTodo(todo)">{{ todo.title }}</label>
            <File class="filecom" :id="todo.id" :type="todo.fileID ? 1: 0" :fileID="todo.fileID" @uploadSuccess="uploadSuccessHandleItem" />
